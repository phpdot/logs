...
```
   ...
   $tracer->error(...)        builds a correlated        ├─ psr-bridge  → Monolog / PSR-3
   $tracer->span(...)         record per call,           └─ NullWriter  → discarded (default)
   $span->setAttribute(...)   hands it to WriterInterface
```
...
The engine ships with a `NullWriter` bound by default, so it works with nothing else installed (records are discarded). Add [phpdot/tracelog](https://github.com/phpdot/tracelog) or [phpdot/psr-bridge](https://github.com/phpdot/psr-bridge) and bind it to actually persist them.
...
```
src/
├── CoreTracer.php        TracerInterface — channels, spans, correlated logging
├── CoreSpan.php          the span: attributes, events, status, log lines, end()
├── PendingLog.php        the deferred log handle behind ->secure()
├── ScopeManager.php      per-coroutine active-span stack (+ auto-drain)
├── SpanStack.php         the LIFO stack itself
├── TraceKernel.php       the per-request root span boundary
├── NullWriter.php        the default no-op backend
├── Trace/                TraceId (UUIDv7), SpanId, SpanContext, Traceparent (W3C)
└── Enum/                 SpanKind, SpanStatus
```
